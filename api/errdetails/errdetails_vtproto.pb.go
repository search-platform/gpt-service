...
func (m *BadRequest) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.FieldViolations) > 0 {
		for iNdEx := len(m.FieldViolations) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.FieldViolations[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func (m *BadRequest_FieldViolation) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: BadRequest_FieldViolation: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: BadRequest_FieldViolation: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 2:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Type |= BadRequest_FieldViolationType(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *BadRequest) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			if err := m.FieldViolations[len(m.FieldViolations)-1].UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
